...
export default function MessageFeed({ messages, collapsedMessages, busy, editing, editTitle, editCategory, editText, editArticleTime, openMenu, setEditing, setEditTitle, setEditCategory, setEditText, setEditArticleTime, setOpenMenu, toggleMessage, saveEdit, setReadStatus, setPendingDelete, hasFilters }) {
    ...
    {messages.map((message) => {
      ...
      return <article className={`message-row${collapsed ? ' collapsed' : ''}${message.isRead ? '' : ' unread'}`} key={message.id}>
        <div className="message-main">
          {editing === message.id ? <div className="edit-fields"><label className="edit-title-field"><span>Title</span><input value={editTitle} onChange={(event) => setEditTitle(event.target.value)} placeholder="Optional article title" autoFocus /></label><label className="edit-title-field"><span>Category</span><input value={editCategory} onChange={(event) => setEditCategory(event.target.value)} placeholder="Optional category" /></label><textarea className="edit-textarea" value={editText} onChange={(event) => setEditText(event.target.value)} /><label><span>Article date</span><input type="date" value={editArticleTime} onChange={(event) => setEditArticleTime(event.target.value)} /></label></div> : <>{message.title && !collapsed && <h3 className="message-title">{message.title}</h3>}{!collapsed && <p>{message.text}</p>}{collapsed && <p className="collapsed-label">{message.title || 'Message collapsed'}</p>}{(message.category || message.author.length || message.source.length || message.articleTime) && <div className="message-metadata">{message.category && <span>Category: {message.category}</span>}{message.author.length > 0 && <span>By {message.author.join(', ')}</span>}{message.source.length > 0 && <span>Source: {message.source.join(', ')}</span>}{message.articleTime && <span>Article: {dateLabel(message.articleTime)}</span>}</div>}</>}
